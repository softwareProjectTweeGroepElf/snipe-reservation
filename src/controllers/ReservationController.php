...
class ReservationController extends Controller
{
	...
	public function lsaction(Request $request){
		...
		$status = DB::table('assets')->where('id', $asset_id)->get()[0]->assigned_to;

		if($action == "checkout"){
			
			if (!$status == null){return "ERROR: ASSET ISN'T AVAILABLE!";} 

			DB::table('assets')
            ->where('id', $asset_id)
            ->update(['assigned_to' => $user_id]);

            return "Succesfully assigned asset with ID ". $asset_id." to user with ID ". $user_id;
		}
